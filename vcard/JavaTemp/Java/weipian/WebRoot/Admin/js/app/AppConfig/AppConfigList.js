var AppConfigConfig={
	del:function(fdId){
		...
		if(fdId){
			...
			var fdIds=AppConfigConfig.table.getSelectedIndex();
			if(fdIds.length==0){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"AppConfigAction/delete",
	        data:params,
	        ...
	        success: function(data){
	        	if(data.success){
	        		...
	        		AppConfigConfig.table.fnPageChange(0);
	        	}else{
	...
	load:function(){
		AppConfigConfig.table = $('#tableList').sysTable({
			url:hxltUrl+'AppConfigAction/getList',
			formId:'.search-form',
	        "aoColumns" : [  {
						"sWidth" : "auto",
						...
						"fnRender":function(obj){
							...
							return '<a href="'+hxltUrl+'AppConfigAction/toEdit?fdId='+obj.aData.fdId+'" class="button">编辑</a>' +
									'<a href="javascript:;" onclick="RetailerConfig.enableOrDisabled(\''+obj.aData.fdId+'\',\''+obj.aData.fdStatus+'\')" class="button">'+fdStatus+'</a>';
						}
					}
			]
	    });  
	...
	enableOrDisabled:function(fdId,fdStatus){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"AppConfigAction/modifStatus",
	        data:params,
	        contentType:'application/x-www-form-urlencoded;charset=UTF-8',
	        dataType: "json",
	        success: function(data){
