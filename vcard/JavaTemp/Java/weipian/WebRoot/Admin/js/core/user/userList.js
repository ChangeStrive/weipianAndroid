var userConfig={
	del:function(fdId){
		...
		if(fdId){
			...
			var fdIds=userConfig.table.getSelectedIndex();
			if(fdIds.length==0){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"SysUserAction/delete",
	        data:params,
	        ...
	        success: function(data){
	        	if(data.success){
	        		...
	        		userConfig.table.fnPageChange(0);
	        	}else{
	...
	load:function(){
		userConfig.table = $('#tableList').sysTable({
			url:hxltUrl+'SysUserAction/getList',
			formId:'.search-form',
	        "aoColumns" : [  {
						"sWidth" : "auto",
						...
						"sWidth" : "120",
						"fnRender":function(obj){
							var fdStatus = obj.aData.fdIsAdmin== "1" ? "<span class='label label-success'>是</span>" : "<span class='label label-error'>否</span>";
							return fdStatus;
						}
					},{
						"sWidth" : "auto",
						...
						"fnRender":function(obj){
							...
							return '<a href="'+hxltUrl+'SysUserAction/toEdit?fdId='+obj.aData.fdId+'" class="button">编辑</a><a href="javascript:;" onclick="userConfig.enableOrDisabled(\''+obj.aData.fdId+'\',\''+obj.aData.fdStatus+'\')" class="button">'+fdStatus+'</a><a onclick="userConfig.del(\''+obj.aData.fdId+'\')" href="javascript:;" class="button">删除</a>';
						}
					}
			]
	    });  
	...
	enableOrDisabled:function(fdId,fdStatus){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"SysUserAction/modifStatus",
	        data:params,
	        contentType:'application/x-www-form-urlencoded;charset=UTF-8',
	        dataType: "json",
	        success: function(data){
