var WebUserConfig={
	del:function(fdId){
		...
		if(fdId){
			...
			var fdIds=WebUserConfig.table.getSelectedIndex();
			if(fdIds.length==0){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"WebUserAction/delete",
	        data:params,
	        ...
	        success: function(data){
	        	if(data.success){
	        		...
	        		WebUserConfig.table.fnPageChange(0);
	        	}else{
	...
	load:function(){
		WebUserConfig.table = $('#tableList').sysTable({
			url:hxltUrl+'WebUserAction/getList',
			formId:'.search-form',
	        "aoColumns" : [  {
						"sWidth" : "auto",
						...
						"sWidth" : "auto",
						"fnRender":function(obj){
							var fdSex = obj.aData.fdSex == '1' ? '男' : '女';
							return fdSex;
						}
					},{
						"mDataProp" : "fdTel",
						...
						"mDataProp" : "fdLastLoginIp",
						"sWidth" : "auto"
					},{
						"mDataProp" : "fdLastLoginTime",
						"sWidth" : "auto"
					},{
						"mDataProp" : "fdLoginCount",
						"sWidth" : "auto"
					}*/,{
						...
						"fnRender":function(obj){
							...
							return '<a href="'+hxltUrl+'WebUserAction/toEdit?fdId='+obj.aData.fdId+'" class="button">编辑</a><a href="javascript:;" onclick="WebUserConfig.enableOrDisabled(\''+obj.aData.fdId+'\',\''+obj.aData.fdStatus+'\')" class="button">'+fdStatus+'</a><a onclick="WebUserConfig.del(\''+obj.aData.fdId+'\')" href="javascript:;" class="button">删除</a>';
						}
					}
			]
	    });  
	...
	enableOrDisabled:function(fdId,fdStatus){
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"WebUserAction/modifStatus",
	        data:params,
	        contentType:'application/x-www-form-urlencoded;charset=UTF-8',
	        dataType: "json",
	        success: function(data){
