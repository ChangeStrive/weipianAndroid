$(function(){
	...
	$(".cancelColleBtn").live("click",function(){
		var fdGoodsId=$(this).attr("fdGoodsId");
		cancelCollection(fdGoodsId);
	});
});

function cancelCollection(fdGoodsId){
	var params={};
	params["fdGoodsId"]=fdGoodsId;
	$.ajax({
        type:'POST',
        url:hxltUrl+"wxajax/cancelCollection",
        data:params,
...
var indexConfig={
	...
	load:function(me){//网络加载
		...
		$.ajax({
	        type:'POST',
	        url:hxltUrl+"wxajax/collGoodsList",
	        data:params,
	        contentType:'application/x-www-form-urlencoded;charset=UTF-8',
	        dataType: "json",
	        success: function(data){
