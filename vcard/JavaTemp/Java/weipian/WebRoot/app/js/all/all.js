$(function(){
	...
	$(".confirm").live("click",function(){
		...
		if(confirm("确定收货？")){
			indexConfig.sureReceiveGoods(fdOrderId);
		}
	});
	
	$(".applyReturnBtn").live("click",function(){
		var fdOrderId=$(this).attr("fdOrderId");
...
var indexConfig={
	...
	appendHtml:function(item){//数据渲染
		...
		if(item.fdStatus=="0"){
			orderStatus="待付款";
			options=TemplateUtils.getTemplate("#noPay",{fdId:item.fdId});
		}else if(item.fdStatus=="1"){
			orderStatus="等待发货";
			...
			orderStatus="退款申请中";
			options="";
		}else if(item.fdStatus=="2"){
			orderStatus="已发货";
			options=TemplateUtils.getTemplate("#recevieGoods",{fdId:item.fdId,fdIsShow:item.fdIsShow});
		}else if(item.fdStatus=="4"){
			orderStatus="待评价";
		}else if(item.fdStatus=="5"){
			orderStatus="已退款";
		}else if(item.fdStatus=="-2"){
			orderStatus="已退款";
		}else if(item.fdStatus=="-1"){
			orderStatus="已取消";
		}else if(item.fdStatus=="3"){
			orderStatus="待评价";
			options=TemplateUtils.getTemplate("#replyGoods",{fdId:item.fdId,fdIsShow:item.fdIsShow});
		}else if(item.fdStatus=="7"){
			orderStatus="已完成";
		}
		var data={
