...
public class ImageHunter
{
  ...
  public ImageHunter(Map<String, Object> conf, HttpServletRequest request)
  {
  ...
  public State captureRemoteData(HttpServletRequest request,String urlStr)
  {
    ...
    try
    {
      ...
      if ((StringUtil.isNotNull(tomcatDir)) && (tomcatDir.equals("1")))
        fileSavePath = request.getSession().getServletContext().getRealPath("/");
      else {
        fileSavePath = PropertiesUtil.getPropertiesValue("fileSavePath");
      }
      
      
      String savePath = getPath(this.savePath, this.filename, suffix);
      String physicalPath = fileSavePath + savePath;
      
      State state = StorageManager.saveFileByInputStream(connection.getInputStream(), physicalPath);

      String downAction =StringUtil.getCurrentUrl(request, "file/");
      if (state.isSuccess()) {
    	 String urls =downAction+ PathFormat.format(savePath);
        state.putInfo("url",urls);
