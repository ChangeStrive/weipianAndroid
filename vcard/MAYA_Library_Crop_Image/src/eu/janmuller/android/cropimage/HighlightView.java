...
public class HighlightView {
	...
	private static final int COLOR = 0xFF0EA6D7; // 0xFFFF8A00 橙色 //0xFF0EA6D7
	...
	public static final int GROW_LEFT_TOP_EDGE = GROW_NONE | GROW_LEFT_EDGE
			| GROW_TOP_EDGE;
	public static final int GROW_LEFT_BOTTOM_EDGE = GROW_NONE | GROW_LEFT_EDGE
			| GROW_BOTTOM_EDGE;
	public static final int GROW_RIGHT_TOP_EDGE = GROW_NONE | GROW_RIGHT_EDGE
			| GROW_TOP_EDGE;
	public static final int GROW_RIGHT_BOTTOM_EDGE = GROW_NONE
			| GROW_RIGHT_EDGE | GROW_BOTTOM_EDGE;
	...
	private void init() {
		...
		mResizeDrawableWidth = resources.getDrawable(R.drawable.fwex_crop_img_camera_crop_2);
		mResizeDrawableHeight = mResizeDrawableWidth;
		// resources.getDrawable(R.drawable.camera_crop_height);
		mResizeDrawableDiagonal = resources
				.getDrawable(R.drawable.fwex_crop_img_indicator_autocrop);
		this.mResizeDrawableTop = resources
				.getDrawable(R.drawable.fwex_crop_img_camera_crop_1);
		this.mResizeDrawableBottom = this.mResizeDrawableTop;
	}
	...
	protected void draw(Canvas canvas) {
		...
		if (!hasFocus()) {
			...
			int heightHeight = widthHeight;
			int heightWidth = widthWidth;

			int xMiddle = mDrawRect.left + ((mDrawRect.right - mDrawRect.left) / 2);
			int yMiddle = mDrawRect.top + ((mDrawRect.bottom - mDrawRect.top) / 2);
			
			int topWidth = this.mResizeDrawableTop.getIntrinsicWidth() / 2;
			int topHeight = this.mResizeDrawableTop.getIntrinsicHeight() / 2;
			...
			if (mMode == ModifyMode.Grow) {
				if (mCircle) {
					...
					case GROW_NONE | GROW_BOTTOM_EDGE:
						...
						break;
					//左上移
					case GROW_LEFT_TOP_EDGE:
						//左上点
						mResizeDrawableTop.setBounds(left - topWidth, top
								- topHeight, left + topWidth, top + topHeight);
								mResizeDrawableTop.draw(canvas);
						break;
					//右上移
					case GROW_RIGHT_TOP_EDGE:
						//右上点
						mResizeDrawableTop.setBounds(right - topWidth, top
								- topHeight, right + topWidth, top + topHeight);
							mResizeDrawableTop.draw(canvas);
						break;
					//右下移
					case GROW_RIGHT_BOTTOM_EDGE:
						//右下点
						mResizeDrawableBottom.setBounds(right - bottomWidth, bottom
								- bottomHeight, right + bottomWidth, bottom
								+ bottomHeight);
						mResizeDrawableBottom.draw(canvas);
						break;
					//左下移
					case GROW_LEFT_BOTTOM_EDGE:
						//左下点
						mResizeDrawableBottom.setBounds(left - bottomWidth, bottom
								- bottomHeight, left + bottomWidth, bottom
								+ bottomHeight);
						mResizeDrawableBottom.draw(canvas);
						break;
					}
...
//					mResizeDrawableBottom.draw(canvas);
	...
	public void setMode(ModifyMode mode, int edges) {
		this.mEdges = edges;
		this.setMode(mode);
	}
...
//					(((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * yDelta, edge);
	...
	void growBy(float dx, float dy, int edges) {
...
//		r.inset(-dx, -dy);
		...
		//右上移
		case GROW_RIGHT_TOP_EDGE:
			r.set(r.left, r.top - dy, r.right + dx, r.bottom);
			break;
		//右下移
		case GROW_RIGHT_BOTTOM_EDGE:
		...
		//左下移
		case GROW_LEFT_BOTTOM_EDGE:
			r.set(r.left + dx, r.top, r.right, r.bottom - dy);
			break;
		}
